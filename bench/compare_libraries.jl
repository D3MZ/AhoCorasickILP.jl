# Compares AhoCorasickILP against the registered pure-Julia `AhoCorasick.jl` (v0.1.1).
#
...
#
#     julia -e 'using Pkg; Pkg.add("AhoCorasickILP"); Pkg.add("AhoCorasick")'
...
hasaho = try
    ...
    @info "AhoCorasick.jl not installed; showing AhoCorasickILP only. `Pkg.add(\"AhoCorasick\")` to compare."
    false
end
...
for approx in sizes
    ...
    GC.@preserve data begin
        ...
        @printf("%-24s %9d %11.4f %10.1f %13d %10d\n", "AhoCorasickILP serial", n, ts/1e6, n/ts*1e3, 0, fc)
        @printf("%-24s %9d %11.4f %10.1f %13d %10d\n", "AhoCorasickILP ILP x8", n, ti/1e6, n/ti*1e3, 0, fc)
    end
    if hasaho
        ...
        @printf("  -> AhoCorasickILP ILP is %.0fx faster, %d fewer bytes allocated\n", ta/ti, aalloc)
    end
    println()
end
